...
final class LayoutStoreTests: XCTestCase {
    ...
    func testFreshDefaultLayoutMatchesRecommendedMetricSections() {
        ...
        // Claude: Session + Weekly stay primary (matching Codex); the rest go below the caret (a provider
        // always keeps ≥1 primary row) — see AGENTS.md "## Providers".
        XCTAssertEqual(primaryByProvider["claude"], ["claude.session", "claude.weekly"])
        XCTAssertEqual(expandedByProvider["claude"], [
            "claude.sonnet", "claude.extra", "claude.trend",
