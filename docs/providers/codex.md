...
The Usage Trend's cloud half comes from a best-effort `GET https://chatgpt.com/backend-api/wham/usage/daily-token-usage-breakdown` call (the same API the Codex analytics page uses), summing each day's per-surface credit percentages. The `cli` surface calibrates credits-per-token against the local logs; the remaining surfaces impute tokens at that rate. Any failure — or no overlap to calibrate from — leaves the trend local-only.
