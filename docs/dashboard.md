...
<<<<<<< HEAD
**Usage Trend** (Claude, Codex, Cursor, and Grok) is a small bar chart of the last 30 days of token usage — one bar per day, drawn from the same source as that provider's spend rows (local logs for Claude, Codex, and Grok; Cursor's usage export for Cursor). For Codex, each day also folds in an estimate of your usage on other surfaces (desktop app, web, cloud tasks) from ChatGPT's analytics — hover a day to see the measured local and estimated cloud halves separately (see [the Codex page](providers/codex.md)). **Hover it** for the peak day, the date range, and the source. It's on by default; turn it off or reorder it from Customize like any other metric. It can't be starred for the menu bar — the strip shows single values, not a chart.
=======
