...
final class StatusItemController: NSObject {
    ...
    private lazy var outsideClickMonitor = PanelOutsideClickMonitor(
        ...
        // Also suspended while a native modal dialog (NSOpenPanel from the account settings) is up —
        // the dialog taking key focus, or the click landing on it, must not dismiss the panel.
        isMorphing: { [weak self] in
            guard let self else { return false }
            return self.heightController.isMorphing || self.isModalDialogPresented
        },
    ...
    /// Fed into `outsideClickMonitor` through its suspension closure alongside the height morph.
